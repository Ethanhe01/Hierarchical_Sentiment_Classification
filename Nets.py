...
class AttentionalBiRNN(nn.Module):
    ...
    def __init__(self, inp_size, hid_size, dropout=0, RNN_cell=nn.GRU):
        ...
        self.rnn = RNN_cell(input_size=inp_size, hidden_size=hid_size, num_layers=1, bias=True, batch_first=True,
    ...
    def forward(self, packed_batch):
        ...
        attended = self.emb_att.forward(emb_h, len_s) * enc_sents
...
class UIAttentionalBiRNN(AttentionalBiRNN):
    ...
    def forward(self, packed_batch, user_embs, item_embs):
        ...
        return torch.sum(enc_sents * self.emb_att.forward(summed, len_s), 0)
...
class HAN(nn.Module):
    ...
    def forward(self, batch_reviews, sent_order, ls, lr):
        ...
        sent_embs = self.word.forward(packed_sents)
        ...
        doc_embs = self.sent.forward(packed_rev)
...
class NSCUPA(HAN):
    ...
    def forward(self, batch_reviews, users, items, sent_order, ui_indexs, ls, lr):
        ...
        sent_embs = self.word.forward(packed_sents, emb_u, emb_i)
        rev_embs = self._reorder_sent(sent_embs, sent_order)
        ...
        doc_embs = self.sent.forward(packed_rev, emb_u, emb_i)
...
class UPTHAttentionalBiRNN(nn.Module):

    def __init__(self, inp_size, hid_size, dropout=0, RNN_cell=nn.GRU):
        super(UPTHAttentionalBiRNN, self).__init__()
...
class UITHAttentionalBiRNN(UPTHAttentionalBiRNN):

    def __init__(self, inp_size, hid_size, dropout=0, RNN_cell=nn.LSTM):
        super(UITHAttentionalBiRNN, self).__init__(inp_size, hid_size, dropout, RNN_cell)

        self.register_buffer("mask", torch.FloatTensor())
        self.att_h = nn.Linear(inp_size * 4 + self.natt, self.natt, bias=True)

    def forward(self, packed_batch, user_embs, item_embs, time_embs, helpful_embs):
        rnn_sents, _ = self.rnn(packed_batch)
        enc_sents, len_s = torch.nn.utils.rnn.pad_packed_sequence(rnn_sents)

        uit = torch.cat(
            [
                user_embs.expand_as(enc_sents),
                item_embs.expand_as(enc_sents),
                time_embs.expand_as(enc_sents),
                helpful_embs.expand_as(enc_sents),
                enc_sents
            ],
            dim=-1
        )
...
class NSCUPTHA(HAN):
    ...
    def __init__(self, ntoken, nusers, nitems, ntimes, nhelpfuls, num_class, emb_size=200, hid_size=100):
        super(NSCUPTHA, self).__init__(ntoken, num_class, emb_size, hid_size)

        self.users = nn.Embedding(nusers, emb_size)
        I.normal(self.users.weight.data, 0.01, 0.01)
        self.items = nn.Embedding(nitems, emb_size)
        I.normal(self.items.weight.data, 0.01, 0.01)
        self.times = nn.Embedding(ntimes, emb_size)
        I.normal(self.times.weight.data, 0.01, 0.01)
        self.helpfuls = nn.Embedding(nhelpfuls, emb_size)
        I.normal(self.helpfuls.weight.data, 0.01, 0.01)

        self.word = UITHAttentionalBiRNN(emb_size, emb_size // 2)
        self.sent = UITHAttentionalBiRNN(emb_size, emb_size // 2)

    def forward(self, batch_reviews, users, items, times, helpfuls, sent_order, ui_indexs, ls, lr):
        u = users[ui_indexs]
        i = items[ui_indexs]
        t = times[ui_indexs]
        h = helpfuls[ui_indexs]

        emb_w = F.dropout(self.embed(batch_reviews), training=self.training)
        emb_u = F.dropout(self.users(u), training=self.training)
        emb_i = F.dropout(self.items(i), training=self.training)
        emb_t = F.dropout(self.times(t), training=self.training)
        emb_h = F.dropout(self.helpfuls(h), training=self.training)

        packed_sents = torch.nn.utils.rnn.pack_padded_sequence(emb_w, ls, batch_first=True)

        sent_embs = self.word.forward(packed_sents, emb_u, emb_i, emb_t, emb_h)
        rev_embs = self._reorder_sent(sent_embs, sent_order)
        ...
        emb_u = F.dropout(self.users(users), training=self.training)
        emb_i = F.dropout(self.items(items), training=self.training)
        emb_t = F.dropout(self.times(times), training=self.training)
        emb_h = F.dropout(self.helpfuls(helpfuls), training=self.training)

        doc_embs = self.sent.forward(packed_rev, emb_u, emb_i, emb_t, emb_h)
...
class UITHWAttentionalBiRNN(AttentionalBiRNN):

    def __init__(self, inp_size, hid_size, dropout=0, RNN_cell=nn.LSTM):
        super(UITHWAttentionalBiRNN, self).__init__(inp_size, hid_size, dropout, RNN_cell)

        self.register_buffer("mask", torch.FloatTensor())
        self.att_h = nn.Linear(inp_size * 2 + self.natt, self.natt, bias=True)

    def forward(self, packed_batch, user_embs, item_embs, time_embs, helpful_embs, aword_embs):
        rnn_sents, _ = self.rnn(packed_batch)
        enc_sents, len_s = torch.nn.utils.rnn.pad_packed_sequence(rnn_sents)
        ...
        uit = torch.cat(
            [
                ...
                time_embs.expand_as(enc_sents),
                helpful_embs.expand_as(enc_sents),
                aword_embs.expand_as(enc_sents),
                enc_sents
            ],
            dim=-1
        )
...
class NSCUPTHWA(HAN):
    ...
    def __init__(self, ntoken, nusers, nitems, ntimes, nhelpfuls, nawords, num_class, emb_size=200, hid_size=100):
        super(NSCUPTHWA, self).__init__(ntoken, num_class, emb_size, hid_size)
        ...
        self.helpfuls = nn.Embedding(nhelpfuls, emb_size)
        I.normal(self.helpfuls.weight.data, 0.01, 0.01)
        self.awords = nn.Embedding(nawords, emb_size)
        I.normal(self.awords.weight.data, 0.01, 0.01)

        self.word = UITHWAttentionalBiRNN(emb_size, emb_size // 2)
        self.sent = UITHWAttentionalBiRNN(emb_size, emb_size // 2)

    def forward(self, batch_reviews, users, items, times, helpfuls, awords, sent_order, ui_indexs, ls, lr):
        u = users[ui_indexs]
        i = items[ui_indexs]
        t = times[ui_indexs]
        h = helpfuls[ui_indexs]
        ...
        emb_t = F.dropout(self.times(t), training=self.training)
        emb_h = F.dropout(self.helpfuls(h), training=self.training)
        emb_aw = F.dropout(self.awords(aw), training=self.training)

        packed_sents = torch.nn.utils.rnn.pack_padded_sequence(emb_w, ls, batch_first=True)

        sent_embs = self.word.forward(packed_sents, emb_u, emb_i, emb_t, emb_h, emb_aw)
        rev_embs = self._reorder_sent(sent_embs, sent_order)
        ...
        emb_t = F.dropout(self.times(times), training=self.training)
        emb_h = F.dropout(self.helpfuls(helpfuls), training=self.training)
        emb_aw = F.dropout(self.awords(awords), training=self.training)

        doc_embs = self.sent.forward(packed_rev, emb_u, emb_i, emb_t, emb_h, emb_aw)
