...
def tuple_batch(l):
    user, item, review, rating, summary, helpful, time = zip(*l)
    # print(user, item, rating, helpful)
    r_t = torch.Tensor(rating).long()
    u_t = torch.Tensor(user).long()
    i_t = torch.Tensor(item).long()
    t_t = torch.Tensor(time).long()
    h_t = torch.Tensor(helpful).long()
    list_rev = review
    ...
    return batch_t,r_t,u_t,i_t,t_t,h_t,sent_order,ui_indexs,ls,lr,review
...
def train(epoch,net,optimizer,dataset,criterion,cuda):
    ...
    with tqdm(total=len(dataset),desc="Training") as pbar:
        for iteration, (batch_t,r_t,u_t,i_t,t_t,h_t,sent_order,ui_indexs,ls,lr,review) in enumerate(dataset):

            data = tuple2var(data_tensors,(batch_t,r_t,u_t,i_t,t_t,h_t,sent_order,ui_indexs))
            optimizer.zero_grad()
            out = net(data[0],data[2],data[3],data[4],data[5],data[6],data[7],ls,lr)
...
def load(args):
    ...
    time_mapping = data_tl.get_field_dict("reviewTime",key_iter=trainit,offset=1) #creates class mapping
    data_tl.set_mapping("reviewTime",time_mapping,unk=0)
    time_mapping["_unk_"] = 0

    helpful_mapping = data_tl.get_field_dict("helpful",key_iter=trainit,offset=1) #creates class mapping
    data_tl.set_mapping("helpful",helpful_mapping,unk=0)
    helpful_mapping["_unk_"] = 0
    ...
    if args.load:
        net = NSCUPTHA(
            ntoken=len(state["word_dic"]),
            nusers=state["users.weight"].size(0),
            nitems=state["items.weight"].size(0),
            ntimes=state["times.weight"].size(0),
            nhelpfuls=state["helpfuls.weight"].size(0),
            emb_size=state["embed.weight"].size(1),
            hid_size=state["sent.rnn.weight_hh_l0"].size(1),
        ...
        if args.emb:
            net = NSCUPTHA(
                ...
                ntimes=len(time_mapping),
                nhelpfuls=len(helpful_mapping),
                emb_size=len(tensor[1]),
                hid_size=args.hid_size,
                num_class=len(rating_mapping)
            )
