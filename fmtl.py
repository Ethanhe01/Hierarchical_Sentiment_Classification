...
class FMTL():
    ...
    def __init__(self, tuplelist,rows=None):
        ...
        self.unknown = {}
        self.rows = rows
        print("in rows: ", rows, "self rows:", self.rows)
    ...
    def _f2i(self,field):
        print(field, self.rows)
        if type(field) == int:
            return int(field)

        if type(field) == str:
            print(field, self.rows[field])
            return int(self.rows[field])
    ...
    def field_gen(self, field, key_iter=None):
        ...
        for idx in key_iter:
            if isinstance(self[idx][field], list):
                #print(field, self[idx][field])
                yield "none"
            else:
